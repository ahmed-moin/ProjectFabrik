...
int main()
{
  try
  {
    ...
    for (x = 0; x <= width; x += step_x)
      pen.Line(imagewidth - x - 1, 0, imagewidth - x - 1, 8);

    // Beschriftung
    step_x = (int)(width * step_n / (end - start));
    int align = gd::Pen::RIGHT | gd::Pen::TOP;
    for (x = 0, n = start;
         start > end ? (n > end + step_n) : (end + step_n > n);
         x += step_x, n += step_n)
    {
      pen.MoveTo(imagewidth - x - 1, 11)
         .WriteAlign(align, to_string(n));

      align = gd::Pen::HCENTER | gd::Pen::TOP;
    }

    pen.MoveTo(0, 11 + end_rect.GetHeight() / 2)
     .WriteAlign(gd::Pen::LEFT | gd::Pen::TOP, unit);
